<div class="col-12">
    ...
    <div class="mt-2 mb-3 d-flex justify-content-between align-items-center">
        <x-link :href="route('index.odcb')">
            <i class="fa fa-arrow-left"></i> Kembali
        </x-link>
        <button class="btn {{ $disable ? 'btn-danger' : 'btn-success' }}" wire:click="setDisable()">
            <i class="fa fa-ban"></i>
        </button>
    </div>
    <div class="my-2">
    ...
    <x-card>
        @if ($step == 1)
            <div class="row row-cols-lg-4  row-cols-2">
                ...
                <x-input :disable="$disable" :error="$errors->has('nomor_cb')" key='nomor_cb' label="ID Objek"
                    wire:model.blur='form.nomor_cb' />

                <x-select :disable="$disable" label='Peringkat' key='peringkat' wire:model.change='form.peringkat'>
                    <option value="">Pilih Peringkat</option>
                    <option value="1" selected>Tidak Ada</option>
                    <option value="2" selected>Kabupaten</option>
                    <option value="3" selected>Provinsi</option>
                    <option value="4" selected>Nasional</option>
                </x-select>
                ...
                <x-input :disable="$disable" :error="$errors->has('pendaftar')" key='pendaftar' label="Nama Pendaftar"
                    wire:model='form.pendaftar' />

                <x-input :disable="$disable" :error="$errors->has('tgl_pendaftaran')" type="date" key='tgl_pendaftaran'
