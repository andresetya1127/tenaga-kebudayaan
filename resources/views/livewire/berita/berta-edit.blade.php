                ...
                @if ($imgTemp)
                    <div class="row row-cols-1">
                        <div class="col m-auto position-relative" style="width: auto">
                            ...
                            <img src="{{ $imgTemp->temporaryUrl() }}" alt="" class="img-thumbnail"
                                style="max-height: 350px">
                        </div>
                    </div>
                @elseif (!$imgTemp && $image)
                    <div class="row row-cols-1">
                        <div class="col m-auto position-relative" style="width: auto">
                            <button wire:click="deleteImg()" type="button"
                                class="btn btn-danger rounded-circle position-absolute end-0">
                            ...
                            <img src="{{ asset('storage/photos/' . $image) }}" alt="" class="img-thumbnail"
                                style="max-height: 350px">
                        </div>
                    </div>
                @else
        ...
        <div class="col-lg-6">
            <x-card>
                <div class="">
                    ...
                    <x-input place="Masukkan Judul Berita." wire:model='title' key="title" />
                </div>

                <div class="">
                    <label for="" class="form-label">Isi Berita</label>
                    <div wire:ignore>
                        <textarea class="summernote" wire:model='content'>{{ $content ?? '' }}</textarea>
                    </div>
                    <x-default-error key='content'></x-default-error>
                </div>
